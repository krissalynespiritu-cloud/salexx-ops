...
| 36 | `36_material_paid.sql` | Adds `paid` / `paid_on` to `job_costs` and a `job_material_status` view. Backs per-job material line items with a Paid checkbox on the Job Costing tab, and the "Materials unpaid" figure on the dashboard. Run any time. Safe to re-run. |
