...
| 50 | `50_weekly_lead_sources.sql` | Adds the LEADS & SOURCES breakdown the Weekly Entry sheet tracks next to New Leads (Total) but `weekly_metrics` never had: `reached_out`, `facebook_social`, `referral`, `website`, `phone_nextdoor_google`. Backfills the 10 weeks migration 39 already imported (Jul 3 - Sep 4 2026 Week Of rows) from the sheet. Backs the new "Leads & sources" group on the Weekly Meeting page and the "Leads by source (weekly log)" table on Performance, alongside (not replacing) the existing per-lead-record source breakdown. Run any time. Safe to re-run. |
