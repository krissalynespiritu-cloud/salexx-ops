-- ============================================================
--  Salexx Ops Hub — reconcile jobs against Monday
--
--  Supabase has more jobs than Monday has items. This makes the two
--  match and shows exactly what differs and why.
--
--  Nothing is deleted. Jobs existing only in Supabase are FLAGGED,
--  not removed — some are real historical projects from the old
--  spreadsheets that were never on the Monday board.
--
--  The board table is PERMANENT, not temporary, so the audit views
--  still work after this finishes.
--
--  It has to run AFTER every step that links a job to a Monday item —
--  monday_sync, resolve_pending_matches, link_pending, cleanup_unlinked,
--  merge_duplicates — because step 1 below is what pulls Monday's
--  stage, price, and other fields onto whichever jobs those steps
--  linked. It also has to run after clients.sql, since step 3 writes
--  to the clients table.
--
--  Run AFTER 18_clients.sql. Safe to re-run.
-- ============================================================

drop table if exists monday_board cascade;
create table monday_board (
...
insert into monday_board values
...
alter table monday_board enable row level security;
drop policy if exists team_all on monday_board;
create policy team_all on monday_board for all to authenticated using (true) with check (true);


-- ---------- 1. Monday wins on every linked job ----------
update jobs j set
  stage            = b.stage::job_stage,
  client_name      = b.client_name,
  address_city     = coalesce(nullif(b.address_city,''), j.address_city),
  crew             = coalesce(b.crew::crew_type, j.crew),
  job_type         = coalesce(nullif(b.job_type,''), j.job_type),
  sold_date        = coalesce(b.sold_date, j.sold_date),
  completed_date   = coalesce(b.completed_date, j.completed_date),
  drive_folder_url = coalesce(nullif(b.drive_folder_url,''), j.drive_folder_url),
  permit_required  = coalesce(b.permit_required, j.permit_required),
  contract_price   = coalesce(j.contract_price, b.contract_price)
from monday_board b
where j.monday_item_id = b.monday_item_id;


-- ---------- 2. Insert Monday items with no job yet ----------
-- Excludes three items deliberately held back for a human decision,
-- not auto-inserted (see 14_resolve_pending_matches.sql and
-- 15_link_pending.sql for why): Casey Wixson's two Monday rows
-- (10741795518, 10741598911) look like one Siding + Painting job
-- entered twice — needs fixing on Monday itself before either can be
-- trusted. Sam Sabin's 12800089499 is a blank row with no type, price,
-- or dates. Without this exclusion, every fresh rebuild would silently
-- insert these as real jobs and undo the hold.
with missing as (
  select b.*, row_number() over (order by b.sold_date nulls last, b.client_name) as rn
  from monday_board b
  where not exists (select 1 from jobs j where j.monday_item_id = b.monday_item_id)
    and b.monday_item_id not in ('10741795518','10741598911','12800089499')
),
base as (
  select coalesce(max(substring(job_id from 5)::int), 0) as n
  from jobs where job_id ~ '^SLX-[0-9]+$'
)
...
select 'SLX-' || lpad((base.n + m.rn)::text, 3, '0'),
       m.client_name, m.stage::job_stage, nullif(m.job_type,''),
       nullif(m.address_city,''), m.crew::crew_type, m.contract_price,
       m.sold_date, m.completed_date, nullif(m.drive_folder_url,''),
       m.permit_required, m.monday_item_id, 12
from missing m, base;


-- ---------- 3. Relink clients ----------
insert into clients (name, first_job_date)
select min(trim(client_name)), min(sold_date)
from jobs where client_name is not null and trim(client_name) <> ''
...
where lower(trim(j.client_name)) = lower(trim(c.name))
  and (j.client_id is null or j.client_id <> c.client_id);


-- ============================================================
--  4. Audit views. Query these after running.
-- ============================================================

-- Jobs with no Monday item. Anything with hours or costs logged is
-- real work — keep it. Empty ones are probably sync duplicates.
create or replace view jobs_not_on_monday as
select
  j.job_id, j.client_name, j.stage, j.job_type, j.contract_price,
  j.sold_date, j.completed_date,
  (select count(*) from time_entries t where t.job_id = j.job_id) as time_entries,
  (select count(*) from job_costs   c where c.job_id = j.job_id) as cost_rows,
  case
    when exists (select 1 from time_entries t where t.job_id = j.job_id)
      or exists (select 1 from job_costs c where c.job_id = j.job_id)
      then 'HAS DATA - keep'
    when j.contract_price is null then 'empty - likely safe to delete'
    else 'has a price - check before deleting'
  end as verdict
from jobs j
where j.monday_item_id is null
order by j.client_name;

-- Clients with several jobs where at least one has no Monday link.
...
select
  (select count(*) from monday_board) as monday_items,
  (select count(*) from jobs)         as supabase_jobs,
  (select count(*) from jobs where monday_item_id is not null) as linked,
  (select count(*) from jobs where monday_item_id is null)     as not_on_monday,
  (select count(*) from monday_board b
     where not exists (select 1 from jobs j
                       where j.monday_item_id = b.monday_item_id)) as missing_from_app;

-- Stage counts, app vs board. These should be identical afterwards.
create or replace view stage_check as
select
  coalesce(j.stage::text, b.stage) as stage,
  count(distinct j.job_id) filter (where j.monday_item_id is not null) as app_linked,
  count(distinct b.monday_item_id) as on_monday
from monday_board b
full join jobs j on j.stage::text = b.stage
group by 1
